...
const STAT_CARDS = [
  { label: "Modules Active",  value: "7",   sub: "All modules unlocked",    icon: Zap,    color: "text-blue-400",    bg: "bg-blue-500/10",    bar: "bg-blue-500"    },
  { label: "Day Streak",      value: "7",   sub: "↑ 2 from last week",       icon: Flame,  color: "text-emerald-400", bg: "bg-emerald-500/10", bar: "bg-emerald-500" },
  { label: "Problems Solved", value: "42",  sub: "DSA + SQL combined",        icon: Target, color: "text-amber-400",   bg: "bg-amber-500/10",   bar: "bg-amber-500"   },
  { label: "Interview Score", value: "8.4", sub: "Last session avg /10",      icon: Trophy, color: "text-violet-400",  bg: "bg-violet-500/10",  bar: "bg-violet-500"  },
];

const MODULES = [
  { path: "/dsa",       label: "DSA Visualizer",  icon: GitBranch,    desc: "Animate sorting, trees, and graphs with C++ code",  color: "text-emerald-400", bg: "bg-emerald-500/10", borderHover: "hover:border-emerald-500/50", status: "Active", statusColor: "text-emerald-400 bg-emerald-500/10" },
  { path: "/sql",       label: "SQL Playground",  icon: Database,     desc: "Live SQL in browser using WebAssembly + Chart.js",   color: "text-cyan-400",    bg: "bg-cyan-500/10",    borderHover: "hover:border-cyan-500/50",    status: "Active", statusColor: "text-emerald-400 bg-emerald-500/10" },
  { path: "/os",        label: "OS Simulator",    icon: Cpu,          desc: "FCFS, Round Robin, Gantt charts live",               color: "text-amber-400",   bg: "bg-amber-500/10",   borderHover: "hover:border-amber-500/50",   status: "Active", statusColor: "text-amber-400 bg-amber-500/10"   },
  { path: "/review",    label: "AI Code Review",  icon: Code2,        desc: "Gemini AI scores your code quality 1-10",            color: "text-green-400",   bg: "bg-green-500/10",   borderHover: "hover:border-green-500/50",   status: "Active", statusColor: "text-emerald-400 bg-emerald-500/10" },
  { path: "/analytics", label: "Analytics",       icon: BarChart3,    desc: "GitHub stats in Power BI style dashboard",           color: "text-pink-400",    bg: "bg-pink-500/10",    borderHover: "hover:border-pink-500/50",    status: "Active", statusColor: "text-emerald-400 bg-emerald-500/10" },
  { path: "/interview", label: "Interview Coach", icon: MessageSquare,desc: "AI asks questions, scores your answers",             color: "text-violet-400",  bg: "bg-violet-500/10",  borderHover: "hover:border-violet-500/50",  status: "New",    statusColor: "text-violet-400 bg-violet-500/10"  },
  { path: "/portfolio", label: "Portfolio Gen",   icon: Sparkles,     desc: "AI writes your portfolio from a form",               color: "text-rose-400",    bg: "bg-rose-500/10",    borderHover: "hover:border-rose-500/50",    status: "New",    statusColor: "text-rose-400 bg-rose-500/10"      },
];
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      {/* ── GREETING ── */}
      <div>
        <h1 className="text-2xl font-semibold text-white">{greeting}, Arjun 👋</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {STAT_CARDS.map(function(card) {
          ...
          return (
            <div
              key={card.label}
              className="relative overflow-hidden rounded-xl p-5 bg-slate-900 border border-slate-800 hover:border-slate-700 transition-all duration-200"
            >
          ...
          {MODULES.map(function(mod) {
            ...
            return (
              <Link
                key={mod.path}
                to={mod.path}
                className={"block p-4 rounded-xl bg-slate-900 border border-slate-800 " + mod.borderHover + " hover:-translate-y-0.5 transition-all duration-200 cursor-pointer group"}
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <ArrowRight size={13} className="text-slate-600 group-hover:text-slate-400 group-hover:translate-x-0.5 transition-all" />
                </div>
              </Link>
            );
          })}
        ...
        {QUICK_ACTIONS.map(function(action) {
          return (
            <Link
              key={action.path}
              to={action.path}
              className={"flex items-center justify-center gap-2 py-3 rounded-xl border text-sm font-medium transition-all duration-200 " + action.color}
            >
              {action.label}
              <ArrowRight size={13} />
            </Link>
