...
const NAV_ITEMS = [
  ...
  { path: "/achievements", label: "Achievements",     icon: Medal,           badge: "🏆"  },
...
export default function Sidebar({
  ...
  return (
    <aside className={
      "fixed left-0 top-0 h-full z-40 bg-slate-900 border-r border-slate-800 " +
          ...
          return (
            <NavLink
              ...
              className={function(obj) {
                return (
                  "flex items-center gap-3 px-2 py-2.5 rounded-lg transition-all cursor-pointer " +
                  (obj.isActive
          ...
          <div className="bg-slate-800 rounded-xl p-3 border border-slate-700">
            ...
            {nextLevel && (
              <p className="text-[10px] text-slate-600 mt-1.5 text-center">
                {nextLevel.minXP - (totalXP || 0)} XP to {nextLevel.title}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="px-3 pb-4 flex-shrink-0">
          {user ? (
            ...
            <div className="flex items-center gap-2 bg-slate-800 rounded-lg p-2.5">
              <div className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-500 to-violet-500 flex items-center justify-center text-[11px] font-semibold text-white flex-shrink-0">CS</div>
              <div className="min-w-0">
                <p className="text-xs font-medium text-slate-200 truncate">CodeSphere User</p>
